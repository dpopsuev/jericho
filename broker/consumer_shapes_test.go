...
// New: Broker.Pick(Preferences{Role: "investigator"}) → Broker.Spawn → Agent.Perform
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Shape 2: Collective as Agent — N agents behind one interface
// Old: SpawnCollective(staff, 3, strategy) → Collective.Ask
// New: collective.SpawnCollective(ctx, broker, 3, strategy) → Agent.Perform
// ═══════════════════════════════════════════════════════════════════════
...
func TestConsumerShape_CollectiveAsActor(t *testing.T) {
	...
	// SpawnCollective wraps N actors behind one Agent interface
	...
	// Consumer sees one Agent — ISP compliance
	var a troupe.Agent = actor
	if !a.Ready() {
...
func (o *circuitObserver) PreSpawn(_ context.Context, _ troupe.AgentConfig) error { return nil }
func (o *circuitObserver) PostSpawn(_ context.Context, _ troupe.AgentConfig, _ troupe.Agent, err error) {
	o.mu.Lock()
	o.spawns++
	if err != nil {
		o.errors++
...
func TestConsumerShape_HookObservability(t *testing.T) {
	...
	a1, _ := broker.Spawn(ctx, troupe.AgentConfig{Role: "worker"})
	obs.PostSpawn(ctx, troupe.AgentConfig{Role: "worker"}, a1, nil)
